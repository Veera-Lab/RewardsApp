...
public class RewardsController {
	...
	public List<Double> getCustomerRewardsPerMonth(int customerId) {
		...
		List<Transaction> customerTransLast30Days = customerTransactions.stream()
				.filter(t -> !(t.getTransactionDate().before(startDate30DaysBefore))).collect(Collectors.toList());
		List<Transaction> customerTransLast60Days = customerTransactions.stream()
				.filter(t -> !(t.getTransactionDate().before(startDate60DaysBefore)
						|| t.getTransactionDate().after(startDate30DaysBefore)))
				.collect(Collectors.toList());
		List<Transaction> customerTransLast90Days = customerTransactions.stream()
				.filter(t -> !(t.getTransactionDate().before(startDate90DaysBefore)
						|| t.getTransactionDate().after(startDate60DaysBefore)))
				.collect(Collectors.toList());
	...
	private List<Transaction> getTransactionsForCustomer(int customerId) {
		return transactions.stream().filter(transaction -> transaction.getCustomerId() == customerId)
				.collect(Collectors.toList());
	}
